		...
		if (directoryListing != null) {
			for (File child : directoryListing) {
				...
				try {
					...
					//String filename = transformedFolder + "\\" + child.getName().replaceFirst("[.][^.]+$", "") + "_transformed.graphml";

					// System.out.println("export file: "+filename);
					String filename = archiUtil.transform();
...
/*
	public void analyzeForEaSmells(String directoryPath) throws IOException {
		List<String[]> csvData = new ArrayList<>();
		String[] header = { "filename", "CHATTY_SERVICE", "CYCLIC_DEPENDENCY", "DEAD_COMPONENT", "DENSE_STRUCTURE",
				"MESSAGE_CHAIN", "SHARED_PERSISTENCY", "STRICT_LAYER_VIOLATION" };
		csvData.add(header);
		...
		if (directoryListing != null) {
			for (File child : directoryListing) {
				...
				if (sml5 != null) {
					try {
						String degree = sml5.replaceAll("averageDegree: ", "").replaceAll(";", "");
						System.out.println(degree);
						if (degree == "" || degree == null)
							sml5 = "";
						else if (Double.parseDouble(degree) < 1.75) {
							sml5 = "";
						}
					} catch (Exception e) {
				...
				String[] record1 = { child.getName(), String.valueOf(!sml1.isEmpty()), String.valueOf(!sml2.isEmpty()),
						String.valueOf(!sml3.isEmpty()), String.valueOf(!sml4.isEmpty()),
						String.valueOf(!sml5.isEmpty()), String.valueOf(!sml6.isEmpty()),
						String.valueOf(!sml7.isEmpty()), String.valueOf(!sml8.isEmpty())
