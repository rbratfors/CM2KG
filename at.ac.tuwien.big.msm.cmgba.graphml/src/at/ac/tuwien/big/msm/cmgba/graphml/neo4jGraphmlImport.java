...
public class neo4jGraphmlImport {
	...
	private static String setNodeLabels = "MATCH (n)\r\n"
			...
			+ "YIELD node\r\n"
			+ "RETURN node";
	private static String var = "modified";
	private static String setNodeLabelsVar = "MATCH (n)\r\n"
			+ String.format("CALL apoc.create.addLabels( id(n), [ n.%s ] )\r\n", var)
			+ "YIELD node\r\n"
			+ "RETURN node";
	...
	public neo4jGraphmlImport() {
		importGraphmlFile = "CALL apoc.import.graphml(\"http://localhost:8080/repository/graph/current\", {storeNodeIds: true})";
	}
	...
	public void initializeDeltaGraph() {
		connector.executeQuery(deleteAllNodes);
		connector.executeQuery(importGraphmlFile);
		connector.executeQuery(setNodeLabelsVar);
		connector.executeQuery(setRelationshipLabels);	
	}
